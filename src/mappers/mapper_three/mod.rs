    ...
    fn cpuMapWrite(&mut self, addr: u16, val: u8) -> Option<u32> {
        match addr {
            ...
            0x8000..=0x9FFF => {
                ...
                if addr % 2 == 0 {
                    self.target = (val & 3) as usize;
                    self.prgBankMode = (val >> 6) & 1;
                    self.chrInversion = (val >> 7) & 1;
                }
                else {
                    self.vMemRegs[self.target] = val as u16;
                    ...
                    if self.prgBankMode == 0 {
                        ...
                        self.vPrgBanks[2] = (self.secLastPrgBank * 0x2000) as u16;
                        self.vPrgBanks[3] = (self.lastPrgBank * 0x2000) as u16;
                    }
                    else {
                        self.vPrgBanks[0] = (self.secLastPrgBank * 0x2000) as u16;
                        self.vPrgBanks[1] = (self.vMemRegs[7] & 0x3F) * 0x2000;
